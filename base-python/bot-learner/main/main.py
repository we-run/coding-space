...
from paramiko import PKey, RSAKey, SFTPClient
...
def prepare(c):
    ...
    for stage in c.pre_settings:
        ...
        for chain_step in stage['steps']:
            chain_step[len(chain_step) -
                       1]['conn'] = my_utils.create_conn_chain(chain_step)
    ...
    for stage in c.pre_settings:
        ...
        for chain_step in stage.get('steps'):
            if process_pool is None and not stage.get('sync'):
                process_pool = Pool(len(stage.get('steps')))
                # pass
            ...
            if process_pool is not None:
                print('>>> ASYNC EXCUTE >>>')
                process_pool.apply_async(__run_todo, args=(chain_step,))
            else:
                print('>>> SYNC EXCUTE >>>')
...
def __run_todo(chain_step):
    watchers = my_utils.get_runner_auto_responder(__collect_auto_res(chain_step))
    step = chain_step[len(chain_step) - 1]
    conn = step.get('conn')
    pkey = step.get('pkey')
    actions = step.get('actions')
    if conn is not None:
        if pkey is not None:
            r = conn.run('echo $HOME', hide='stdout')
            if isinstance(r, Result) and r.ok:
                r = re.search(r'(\S+)', '{}'.format(r.tail('stdout', count=1)))
                ssh_dir = r.group(0) + '/.ssh'
                judge_shell = 'if [[ -f {}/{} ]];then echo 1; else echo 0; fi'.format(
                    ssh_dir, os.path.basename(pkey))
                if '1' not in conn.run(judge_shell, hide='stdout').tail('stdout', count=1):
...
def __run_action(conn, actions, watchers):
    for action in actions:
        ...
        if action.get('sh_raw') is not None:
            ...
            r = conn.run(action.get('sh_raw') + "||true", env=action.get('env'), watchers=watchers)
            # promise = conn.run(action.get('sh_raw'), env=action.get('env'), watchers=watchers, hide='stdout', asynchronous=True)
            # r = promise.join()
        ...
        if action.get('sh_file') is not None:
            ...
            promise = conn.run('sh ./' + os.path.basename(action.get('sh_file')) + "||true", env=action.get('env'), watchers=watchers, asynchronous=True)
            r = promise.join()
            ...
            #     promise = conn.run(shf.read(), env=action.get('env'),hide='stdout', watchers=watchers, asynchronous=True)
            #     r = promise.join()
            #     my_utils.BLUE_LOG('<<== FILE SHELL RES <<==', ' Response : {}'.format(r))
